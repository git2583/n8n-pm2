...
### 💡 왜 PM2로 직접 구동하지 않고 manager.js를 만들었는가?
초기에는 Node.js 진영의 표준 프로세스 매니저인 **PM2**를 사용하여 `n8n`과 `cloudflared`를 각각 백그라운드 데몬으로 직접 관리하려 하였습니다. 그러나 **Windows 환경에서 PM2를 사용할 때 발생하는 두 가지 치명적인 결함** 때문에 `manager.js`라는 커스텀 미들웨어(Middleware) 매니저를 직접 개발하게 되었습니다.

**1. 파이프(Pipe) 병목 및 스트림 유실 문제 (EPIPE Error)**
PM2는 내부적으로 Node.js의 클러스터(Cluster) 모듈과 IPC(Inter-Process Communication) 파이프를 통해 자식 프로세스의 `stdout/stderr` 로그를 가로채어 수집합니다. 하지만 n8n과 같이 방대한 로그를 쏟아내거나 cloudflared처럼 외부 바이너리(Go 언어)로 컴파일된 프로그램을 Windows 환경의 PM2로 띄울 경우, 파이프 버퍼가 가득 차거나 통신 연결이 끊어지는 `EPIPE` 에러가 발생하며 서버가 뻗어버리는 현상이 빈번했습니다.

**2. Windows 좀비 프로세스(Zombie Process) 양산 문제**
Windows에서 PM2가 쉘(`.cmd`나 `.bat`) 기반으로 래핑된 n8n을 종료(`pm2 stop n8n`)할 때, 최상단 래퍼(Wrapper) 쉘만 죽이고 실제 구동 중인 알맹이 프로세스는 종료시키지 못하는 구조적 한계가 있습니다. 이로 인해 n8n이 백그라운드에 좀비처럼 남아 포트(`5678`)를 계속 점유하게 되고, 다음 재시작 시 `EADDRINUSE (포트 이미 사용 중)` 에러가 발생하여 시스템 자동 복구(Auto-healing)가 완전히 망가지는 결과를 초래했습니다.

### 🛡️ manager.js와 PM2의 하이브리드 아키텍처 (Comparison)
위 문제들을 근본적으로 해결하기 위해, 우리는 OS 레벨의 안정적인 통제가 가능한 `manager.js`를 개발했습니다.

| 구분 | 순수 PM2 직접 구동 (기존 실패 사례) | **manager.js 커스텀 구동 (현재 성공 구조)** |
| :--- | :--- | :--- |
| **로그 I/O 처리** | IPC 파이프 통신 (Windows에서 병목 및 EPIPE 에러 발생) | **`fs.createWriteStream`을 통한 Direct File I/O (오버헤드 제로)** |
| **프로세스 종료** | 단순 `SIGINT` 전송 (Windows에서 좀비 프로세스 무한 양산) | **`taskkill /PID /T /F` 기반의 완벽한 프로세스 트리 킬링 (좀비 원천 차단)** |
| **타이밍 제어** | 각 프로세스 독립 구동 (브라우저 열림 제어 등 불가) | **스트림 로그 파싱을 통해 구동 100% 완료 시 브라우저 자동 팝업 등 커스텀 로직 가능** |

**결론적으로:** 오류가 잦은 `n8n`과 `cloudflared`의 구동 및 제어(무거운 짐)는 Windows 통제에 최적화된 **`manager.js`**가 짊어지고, **PM2**는 이 가벼워진 `manager.js` 스크립트 단 하나만을 백그라운드에서 실행하고 감시(Watchdog)하도록 역할 분담을 명확히 한 **하이브리드 아키텍처**를 완성했습니다. 이를 통해 관리 포인트를 줄이고 엔터프라이즈급 서버 안정성을 확보했습니다.
